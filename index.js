...
function addKnowledgeRecord(record) {
  ...
  if (record.fields) {
    for (const [key, value] of Object.entries(record.fields)) {
      if (/(part|item|number|model|serial|quote|knife|color|pantone|pms|machine|id|formula|ink)/i.test(key)) {
...
function scoreRecord(record, query) {
  ...
  const terms = qLoose.split(" ").filter((term) => term.length >= 2 && !SEARCH_STOPWORDS.has(term));
  ...
  let matchedTerms = 0;
  for (const term of terms) if (record.normalizedText.includes(term)) matchedTerms++;
  if (matchedTerms > 0) score += matchedTerms * 8 + (matchedTerms === terms.length ? 20 : 0);

  for (const queryKey of queryKeys) {
    for (const recordKey of record.searchKeys || []) {
      if (recordKey === queryKey) score += 120;
      else if (queryKey.length >= 4 && recordKey.length >= 4) {
        const qd = digitsOnly(queryKey);
        const rd = digitsOnly(recordKey);
        if (queryKey[0] === recordKey[0] && qd.length >= 4 && rd.length >= 4 && (recordKey.includes(queryKey) || queryKey.includes(recordKey))) score += 60;
        else if (levenshtein(recordKey, queryKey) <= (Math.max(queryKey.length, recordKey.length) <= 6 ? 1 : 2)) score += 25;
      }
    }
  }
...
function searchKnowledge(query, options = {}) {
  ...
  if (categories.length) records = records.filter((record) => categories.includes(record.category));
  if (!includeGuidance) {
...
function getField(fields, names) {
  ...
  for (const [key, value] of entries) if (wanted.includes(lower(key))) return normalize(value);
  for (const [key, value] of entries) {
    const keyNorm = lower(key).replace(/[^a-z0-9]/g, "");
    for (const name of wanted) if (keyNorm === name.replace(/[^a-z0-9]/g, "")) return normalize(value);
  }
  return "";
}
...
function safeFields(fields, limit = 14) {
  return Object.fromEntries(Object.entries(fields || {}).filter(([key, value]) => !isSensitiveField(key) && normalize(value) !== "").slice(0, limit));
}

function makeExcerpt(body, query, maxLen = 700) {
  const text = normalize(body).replace(/\s+/g, " ");
  if (text.length <= maxLen) return text;
  const terms = normalizeLoose(query).split(" ").filter((term) => term.length > 3 && !SEARCH_STOPWORDS.has(term));
...
function extractInkNumber(query) {
  ...
  if ((q.includes("ink") || isInkFormulaQuestion(q)) && anyNumber) return normalizePart(anyNumber[1]);
  return "";
}

function extractSimplePartInfo(text) {
  const cleaned = normalize(text).replace(/^i need part/i, "").replace(/^request part/i, "").replace(/^add part/i, "").replace(/^need part/i, "").trim();
...
function isInkInventoryRecord(record) {
  ...
  return keys.includes("total weight") || keys.includes("container count") || keys.includes("last count") || keys.includes("weight lb");
}

function isFormulaIdentifierField(fieldName) {
  const k = lower(fieldName);
  return k === "pantone / color" || k === "pantone" || k === "pms" || k === "color" || k === "ink" || k === "formula" || k.includes("ink color number") || k.includes("formula number") || k.includes("pantone");
}
...
function colorValueMatchesInkNumber(value, inkNumber) {
  ...
  if (suffix) return new RegExp("(^|[^0-9])" + digits + "\\s*" + suffix + "($|[^0-9])").test(raw);
...
function findExactInkInventoryRecord(inkNumber) {
  return knowledgeRecords.find((record) => record.category === "03_INK_ROOM" && record.type === "spreadsheet-row" && isInkInventoryRecord(record) && recordColorMatchesInkNumber(record, inkNumber));
}

function findExactInkFormulaRecords(inkNumber) {
  return knowledgeRecords.filter((record) => record.category === "03_INK_ROOM" && record.type === "spreadsheet-row" && !isInkInventoryRecord(record) && recordColorMatchesInkNumber(record, inkNumber));
}

function isInkFormulaQuestion(msg) {
  const q = lower(msg);
  return q.includes("formula") || q.includes("recipe") || q.includes("make more") || q.includes("mix more") || q.includes("mix ink") || q.includes("make ink") || q.includes("ingredients") || q.includes("components") || q.includes("batch") || q.includes("ratio");
}

function isWasteInkQuestion(msg) {
  const q = lower(msg);
  return q.includes("waste ink") || q.includes("waste tote") || q.includes("wastewater") || q.includes("waste water") || q.includes("used ink") || q.includes("ink waste") || q.includes("ink disposal") || (q.includes("ink") && (q.includes("pickup") || q.includes("pick up") || q.includes("picked up") || q.includes("disposal") || q.includes("dispose") || q.includes("environmental") || q.includes("who picks up") || q.includes("who pick up")));
}

function isJonathanStatusQuestion(msg) {
  const q = lower(msg);
  return (q.includes("jonathan") && (q.includes("vacation") || q.includes("back") || q.includes("return") || q.includes("leaving") || q.includes("away") || q.includes("out") || q.includes("phone") || q.includes("number") || q.includes("contact") || q.includes("call") || q.includes("text") || q.includes("reach"))) || q.includes("when is jonathan back") || q.includes("when will jonathan be back") || q.includes("how do i reach jonathan") || q.includes("call jonathan") || q.includes("text jonathan") || q.includes("jonathan's number") || q.includes("jonathan phone") || q.includes("jonathan contact");
}
...
function getPartMatchConfidence(query, record) {
  ...
  for (const queryKey of queryKeys) for (const recordKey of recordKeys) if (queryKey === recordKey) return "exact";
  for (const queryKey of queryKeys) {
    for (const recordKey of recordKeys) {
      const qd = digitsOnly(queryKey);
      const rd = digitsOnly(recordKey);
      if (queryKey.length >= 4 && recordKey.length >= 4 && queryKey[0] === recordKey[0] && qd.length >= 4 && rd.length >= 4 && (recordKey.includes(queryKey) || queryKey.includes(recordKey))) return "likely";
    }
  }
  return "weak";
}
...
function answerInkInventoryRow(top) {
  ...
  const color = getField(fields, ["Ink Color Number", "Color", "Pantone", "PMS"]) || getField(fields, ["Formula Number"]) || "that ink";
...
function answerInkFormulaRow(record) {
  const lines = [];
  for (const [key, value] of Object.entries(safeFields(record.fields || {}, 14))) lines.push(key + ": " + value);
  return lines.length ? lines.join("\n") : "Formula record found, but I could not summarize the fields cleanly.";
}

function answerPartsRow(top) {
  const fields = top.fields || {};
  const partNumber = getField(fields, ["Part Number", "Item Number", "Part", "Item", "Number"]) || getField(fields, ["Normalized Search Key"]);
  ...
  const status = getField(fields, ["Status"]);
  let reply = partNumber || description ? "I found " + (partNumber || "a matching part") + (description ? " — " + description : "") + "." : "I found a matching part.";
...
function answerPartSearchResults(query, results, options = {}) {
  if (!results.length) {
    let reply = "I could not find that part in the current JARVIS parts inventory.\n\nWould you like me to add it to Jonathan's Purchase Order Request list?";
...
function answerMapQuestion(query) {
  ...
  if (q.includes("thermostat") || q.includes("temperature") || q.includes("turn the temperature") || q.includes("turn temperature")) {
    return "Use the thermostat map below to find the thermostat for that area." + imageLine(HVAC_THERMOSTAT_IMAGE) + "\n\nIf the map is unclear, physically verify or ask supervision/Jonathan.";
  }
  if (q.includes("eyewash") || q.includes("eye wash")) return "Eye wash station locations are shown on the map below.\n\nIf this is an active chemical exposure or injury, follow emergency/safety procedures immediately and notify supervision." + imageLine(EYEWASH_IMAGE);
  if (q.includes("fire extinguisher") || q.includes("extinguisher")) return "Fire extinguisher locations are shown on the map below.\n\nIf this is an active fire, smoke, or burning smell situation, follow emergency procedures immediately and notify supervision/emergency services as appropriate." + imageLine(FIRE_EXTINGUISHER_IMAGE);
...
function answerScheduleQuestion() {
  ...
  return "The 2-2-3 schedule is for day shift only and starts June 14, 2026. Night shift remains unchanged.\n\nUse the schedule calendar PDF to check specific days." + (scheduleFile ? linkLine(scheduleFile) : "");
}
...
function answerInkQuestion(query) {
  ...
  const results = searchKnowledge(query, { maxResults: 8, categories: ["03_INK_ROOM"], minScore: 25 });
  ...
  const guidanceResults = searchKnowledge(query, { maxResults: 3, categories: ["03_INK_ROOM"], includeGuidance: true, minScore: 25 });
...
function answerInkFormulaQuestion(query) {
  ...
  if (inkNumber) {
    ...
    if (exactFormulaRows.length) {
      ...
      reply += exactFormulaRows.slice(0, 3).map((record, index) => "Record " + (index + 1) + ":\n" + answerInkFormulaRow(record)).join("\n\n");
...
function answerPartsLookup(query, context = {}) {
  const results = searchKnowledge(query, { maxResults: 6, categories: ["01_PARTS_INVENTORY"], minScore: looksLikePartNumber(query) ? 55 : 80 });
  const candidatePartNumber = extractCandidatePartNumber(query) || query;
  if (!results.length) {
    if (context.from) pendingRequests.set(context.from, { step: "confirm_add_missing_part", requesterName: context.requesterName || "", requesterPhone: context.from, partNumber: candidatePartNumber, partDescription: "", originalQuery: query });
    return "I could not find that part in the current JARVIS parts inventory.\n\nWould you like me to add it to Jonathan's Purchase Order Request list?" + (context.from ? addMissingPartButtons() : "");
  }
  const confidence = getPartMatchConfidence(query, results[0]);
  if (confidence !== "exact" && context.from) pendingRequests.set(context.from, { step: "possible_part_match", requesterName: context.requesterName || "", requesterPhone: context.from, partNumber: candidatePartNumber, partDescription: "", originalQuery: query, possibleMatchSummary: answerPartsRow(results[0]) });
...
function isPurchaseOrderPolicyQuestion(msg) {
  const q = lower(msg);
  return q.includes("po request") || q.includes("por request") || q.includes("purchase order request") || q.includes("blank po") || q.includes("blank por") || q.includes("po form") || q.includes("por form") || q.includes("purchase order form") || q.includes("por-richmond") || q.includes("po process") || q.includes("por process") || q.includes("request a po") || q.includes("request a por") || q.includes("submit a po") || q.includes("submit a por") || q.includes("how do i request a po") || q.includes("how do i request a por") || (q.includes("request form") && (q.includes("po") || q.includes("por") || q.includes("purchase")));
}

function isPartOrderingProcessQuestion(msg) {
  const q = lower(msg);
  return (q.includes("how do i order") || q.includes("how to order") || q.includes("order a part") || q.includes("part we dont have") || q.includes("part we don't have") || q.includes("need a part we dont have") || q.includes("need a part we don't have") || q.includes("request a part") || q.includes("add a part")) && (q.includes("part") || q.includes("parts") || q.includes("bearing") || q.includes("item"));
}

function isPartRequestStart(msg) {
  const q = lower(msg);
  return q.startsWith("i need part") || q.startsWith("need part") || q.startsWith("request part") || q.startsWith("add part");
}
...
function isWrongPartIntent(msg) {
  const q = lower(msg);
  return q === "no" || q === "nope" || q.includes("not the right part") || q.includes("wrong part") || q.includes("not right") || q.includes("not it") || q.includes("not the same") || q.includes("that is not") || q.includes("that's not");
}

function looksLikeDueDateOrUrgency(msg) {
  const q = lower(msg);
  if (q.includes("asap") || q.includes("urgent") || q.includes("today") || q.includes("tomorrow") || q.includes("this week") || q.includes("next week") || q.includes("within 2 weeks") || q.includes("within two weeks") || q.includes("next monday") || q.includes("next tuesday") || q.includes("next wednesday") || q.includes("next thursday") || q.includes("next friday") || q.includes("next saturday") || q.includes("next sunday")) return true;
...
function looksLikeMachineOrArea(msg) {
  ...
  return q.includes("wh1") || q.includes("wh2") || q.includes("wh3") || q.includes("wh4") || q.includes("warehouse") || q.includes("ink room") || q.includes("maintenance") || q.includes("prepress") || q.includes("pre-press") || q.includes("mailshop") || q.includes("shipping") || q.includes("receiving") || q.includes("safety") || q.includes("office") || q.includes("hvac") || q.includes("envelope");
}

function looksLikeNewQuestion(msg) {
  const q = lower(msg);
  if (q.includes("?")) return true;
  if (q.startsWith("where ") || q.startsWith("what ") || q.startsWith("who ") || q.startsWith("when ") || q.startsWith("how ") || q.startsWith("do we ") || q.startsWith("did ") || q.startsWith("does ") || q.startsWith("can ")) return true;
  return isPurchaseOrderPolicyQuestion(q) || isWasteInkQuestion(q) || isJonathanStatusQuestion(q) || q.includes("ink") || q.includes("thermostat") || q.includes("temperature") || q.includes("eyewash") || q.includes("fire extinguisher");
}
...
async function sendTeamsAlert(message) {
  ...
  try {
    await fetch(teamsWebhook, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: message }) });
    return true;
  } catch (error) {
...
async function writePartsRequestToSheet(request) {
  ...
  if (!webhookUrl || !secret) return { ok: false, error: "Missing spreadsheet webhook configuration" };
  const response = await fetch(webhookUrl, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ secret, ...request }) });
...
function startMissingPartRequestFromPending(from, pending, requesterName) {
  pendingRequests.set(from, { step: "awaiting_due_date", requesterName: pending.requesterName || requesterName || "", requesterPhone: from, partNumber: pending.partNumber || pending.originalQuery || "", partDescription: pending.partDescription || "", quantityRequested: "", notes: pending.originalQuery || pending.partNumber || "" });
  return "Okay. I can add this to Jonathan's next Purchase Order Request list.\n\nPart Number: " + (pending.partNumber || pending.originalQuery || "Not provided") + "\nDescription: " + (pending.partDescription || "Not provided") + "\n\nWhat requested due date should I use?" + dueDateButtons() + "\n\nYou can also type your own date, like 6/20 or next Friday.\n\nType cancel if you do not want to create this request.";
}
...
async function handlePendingRequest({ pending, from, cleanBody, requesterName }) {
  ...
  if (pending.step === "possible_part_match") {
    ...
    if (isWrongPartIntent(msg)) {
      pendingRequests.set(from, { ...pending, step: "confirm_add_missing_part" });
      return "Got it. I will treat the original part as not found:\n\n" + (pending.partNumber || pending.originalQuery || "Original part not provided") + "\n\nWould you like me to add it to Jonathan's Purchase Order Request list?" + addMissingPartButtons();
    }
    if (isYesIntent(msg)) { pendingRequests.delete(from); return "Okay — please physically verify that possible match before relying on it for production. What can I help you with next?"; }
    if (isCancelIntent(msg)) { pendingRequests.delete(from); return "Okay — I cleared that possible part match. What can I help you with?"; }
    if (looksLikeNewQuestion(cleanBody)) { pendingRequests.delete(from); return "I cleared the previous possible part match and answered your new question instead.\n\n" + await getJarvisReply({ from, body: cleanBody, requesterName }); }
    return "I need to know whether that possible part match is correct first." + partMatchButtons();
  }
  ...
  if (pending.step === "confirm_add_missing_part") {
    ...
    if (isCancelIntent(msg)) { pendingRequests.delete(from); return "Okay — I will not add that part request. What can I help you with?"; }
    if (looksLikeNewQuestion(cleanBody)) { pendingRequests.delete(from); return "I cleared the pending add-part offer and answered your new question instead.\n\n" + await getJarvisReply({ from, body: cleanBody, requesterName }); }
    return "Please choose whether to add the missing part to Jonathan's Purchase Order Request list." + addMissingPartButtons();
  }

  if (isCancelIntent(msg)) { pendingRequests.delete(from); return "Okay — I canceled the pending parts request. What can I help you with?"; }
  ...
  if (pending.step === "awaiting_due_date") {
    if (looksLikeDueDateOrUrgency(cleanBody)) {
      ...
      return "Got it. What machine or area is this part for?" + machineAreaButtons() + "\n\nYou can also type another machine or area.";
    }
    if (looksLikeNewQuestion(cleanBody)) { pendingRequests.delete(from); return "I paused the pending parts request and answered your new question instead.\n\n" + await getJarvisReply({ from, body: cleanBody, requesterName }); }
    return "I am waiting for the requested due date for the pending part request." + dueDateButtons() + "\n\nYou can also type something like 6/20, next Friday, ASAP, today, tomorrow, or within 2 weeks.\n\nOr type cancel to stop this request.";
  }

  if (pending.step === "awaiting_machine") {
    if (looksLikeNewQuestion(cleanBody)) { pendingRequests.delete(from); return "I paused the pending parts request and answered your new question instead.\n\n" + await getJarvisReply({ from, body: cleanBody, requesterName }); }
    if (!looksLikeMachineOrArea(cleanBody)) return "I am waiting for the machine or area for the pending part request." + machineAreaButtons() + "\n\nYou can also type another machine or area.\n\nOr type cancel to stop this request.";
...
async function startPartRequest({ from, cleanBody, requesterName }) {
  ...
  const lookupResults = searchKnowledge(partNumber || cleanBody, { maxResults: 3, categories: ["01_PARTS_INVENTORY"], minScore: 55 });
  if (lookupResults.length) {
    const confidence = getPartMatchConfidence(partNumber || cleanBody, lookupResults[0]);
    if (confidence !== "exact") {
      pendingRequests.set(from, { step: "possible_part_match", requesterName, requesterPhone: from, partNumber: partNumber || info.partNumber, partDescription: info.partDescription || "", originalQuery: cleanBody, possibleMatchSummary: answerPartsRow(lookupResults[0]) });
      return answerPartSearchResults(partNumber || cleanBody, lookupResults, { includeButtons: true });
    }
    pendingRequests.set(from, { step: "confirm_add_missing_part", requesterName, requesterPhone: from, partNumber: partNumber || info.partNumber, partDescription: info.partDescription || "", originalQuery: cleanBody });
    return "I found an exact match in inventory:\n\n" + answerPartsRow(lookupResults[0]) + "\n\nIf you still need this added to Jonathan's PO request list, choose Add to PO request." + addMissingPartButtons();
  }
  pendingRequests.set(from, { step: "confirm_add_missing_part", requesterName, requesterPhone: from, partNumber: partNumber || info.partNumber, partDescription: info.partDescription || "", originalQuery: cleanBody });
  return "I could not find that part in the current JARVIS parts inventory.\n\nWould you like me to add it to Jonathan's Purchase Order Request list?" + addMissingPartButtons();
}
...
async function getJarvisReply({ from = "browser-test", body = "", requesterName = "" }) {
  ...
  const msg = lower(cleanBody);
  if (knowledgeLoadError) return "JARVIS had a problem loading the knowledge base. Jonathan needs to check the Render logs.";
  ...
  switch (intent) {
    case "help": return "What can I help you with?\n\nYou can ask about parts, ink, ink formulas, waste ink pickup, Jonathan's vacation/contact info, HVAC/thermostats, maps, knives, PO/POR requests, Magna rebuilds, Warehouse 4 mailshop equipment, safety training, or the 2-2-3 schedule.\n\nKnowledge base records loaded: " + knowledgeRecords.length + ".";
    case "jonathan_status": return answerJonathanStatusQuestion(cleanBody);
    case "po_policy": return answerPoRequest();
    case "part_ordering_process": return answerPartOrderingProcess();
    case "part_request_start": return startPartRequest({ from, cleanBody, requesterName });
    case "schedule": return answerScheduleQuestion(cleanBody);
    case "map": return answerMapQuestion(cleanBody);
    case "waste_ink": return answerWasteInkQuestion(cleanBody);
    case "ink_formula": return answerInkFormulaQuestion(cleanBody);
    case "ink": return answerInkQuestion(cleanBody);
    case "parts_lookup": return answerPartsLookup(cleanBody, { from, requesterName });
    case "open_orders": return answerFromTopResults(cleanBody, searchKnowledge(cleanBody, { maxResults: 6, categories: ["02_OPEN_ORDERS"], minScore: 35 }));
    case "magna": return answerFromTopResults(cleanBody, searchKnowledge(cleanBody, { maxResults: 6, categories: ["07_MAGNA_REBUILDS"], minScore: 35 }));
    case "knives": return answerFromTopResults(cleanBody, searchKnowledge(cleanBody, { maxResults: 6, categories: ["05_KNIVES"], minScore: 50 }));
    case "mailshop": return answerFromTopResults(cleanBody, searchKnowledge(cleanBody, { maxResults: 6, categories: ["08_MAILSHOP_EQUIPMENT_OUTGOING"], minScore: 35 }));
    case "safety": return answerFromTopResults(cleanBody, searchKnowledge(cleanBody, { maxResults: 6, categories: ["10_SAFETY_TRAINING"], minScore: 35 }));
    default: {
...
<body>
  <div class="app">
    ...
    <footer class="composer">
      ...
      <div class="input-row"><textarea id="question" placeholder="Ask JARVIS like you would ask Jonathan..."></textarea><button id="askButton" class="send" type="button" onclick="askJarvis()">Ask</button></div>
  ...
  <script>
    ...
    function disableButtonsNear(button) {
      ...
      if (!bubble) return;
      bubble.querySelectorAll(".quick-button").forEach(function(btn) { btn.disabled = true; });
    }
    function renderMessageContent(container, text) {
      ...
      lines.forEach(function(line, index) {
        ...
        if (imageMatch && isSafeImagePath(imageMatch[1])) {
          ...
          button.onclick = function() { disableButtonsNear(button); sendJarvisQuestion(value, label); };
    ...
    async function sendJarvisQuestion(question, displayText) {
      ...
      try {
        const response = await fetch("/api/ask", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ sessionId: getSessionId(), name: name, question: textToSend }) });
    ...
    document.addEventListener("DOMContentLoaded", function() {
      ...
      document.getElementById("question").addEventListener("keydown", function(event) {
        if (event.key === "Enter" && !event.shiftKey) { event.preventDefault(); askJarvis(); }
      });
      document.getElementById("question").focus();
    });
  </script>
...
app.get("/ask", (_req, res) => res.type("html").send(getAskPageHtml()));

app.get("/health", (_req, res) => {
  res.status(200).json({ ok: !knowledgeLoadError, status: "J.A.R.V.I.S. online.", recordsLoaded: knowledgeRecords.length, loadedAt: knowledgeLoadedAt, dataRoot: fs.existsSync(DATA_ROOT) ? "found" : "missing", error: knowledgeLoadError ? knowledgeLoadError.toString() : null });
});

app.get("/kb-status", (_req, res) => {
  const counts = {};
  for (const record of knowledgeRecords) counts[record.category] = (counts[record.category] || 0) + 1;
...
app.get("/test", async (req, res) => {
  const reply = await getJarvisReply({ from: req.query.from || "browser-test", body: req.query.body || "HELP", requesterName: req.query.name || "" });
...
app.post("/sms", async (req, res) => {
  try {
    ...
    if (teamsWebhook) {
      try {
        await fetch(teamsWebhook, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text: "🤖 *J.A.R.V.I.S. SMS*\n**From:** " + from + " (" + city + ", " + state + ")\n**Message:** " + body + "\n\n**JARVIS Reply:** " + reply }) });
      } catch (teamsError) {
