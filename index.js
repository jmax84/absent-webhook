...
const COMMON_SUPPLY_WORDS = [
  "battery", "batteries", "aa battery", "aa batteries", "aaa battery", "aaa batteries", "9v battery", "9v batteries", "9 volt battery", "9 volt batteries",
  "tape", "zip tie", "zip ties", "cable tie", "cable ties", "glove", "gloves",
  "light bulb", "light bulbs", "bulb", "bulbs", "screw", "screws", "bolt", "bolts", "nut", "nuts",
  "marker", "markers", "sharpie", "sharpies", "shop towel", "shop towels", "wipes", "rag", "rags",
  "pen", "pens", "pencil", "pencils", "knife blade", "knife blades", "razor", "razors", "box cutter", "box cutters"
];
...
function isFormulaRecord(record) {
  ...
  return /(formula|percent|component|blue|red|yellow|black|extender|varnish|violet|rubine|warm|process|eclipse|methyl)/i.test(text);
}
...
function findCommonSupplyMatches(query) {
  ...
  for (const record of knowledgeRecords) {
    ...
    // Guardrail: for common supplies, random part-number hits are ignored. Description/common-name text must match.
...
function commonSupplyPrompt(query, userKey) {
  const supplyName = extractCommonSupplyName(query);
    ...
    async function sendMessage(text) {
      ...
      try {
        ...
        if (data.html) {
          ...
          thinking.innerHTML = escapeClientHtml(data.answer).split(String.fromCharCode(10)).join("<br>");
        } else {
...
function safeAnswerFallback(originalMessage) {
  return [
    "I had trouble answering that.",
    "",
    "I flagged this as something Jonathan needs to fix in JARVIS.",
    "",
    `Original question: ${normalize(originalMessage) || "unknown"}`
  ].join("\n");
}
...
app.post("/api/ask", async (req, res) => {
  ...
  try {
    let answer = await processUserMessage(message, userKey);

    if (!answer || answer === "undefined") {
      answer = safeAnswerFallback(message);
      await createEscalation(`JARVIS returned a blank/undefined answer for: ${message}`, {
        priority: "NORMAL",
        notes: "Internal JARVIS answer fallback fired. Debug this question."
      });
    }
    ...
    const answer = safeAnswerFallback(message);
    res.json({
      ok: true,
      version: APP_VERSION,
      answer,
...
app.post("/sms", async (req, res) => {
  ...
  try {
    ...
    if (!answer || answer === "undefined") answer = safeAnswerFallback(message);
  } catch (error) {
